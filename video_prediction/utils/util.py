...
import torch
from tensorflow import broadcast_static_shape, TensorShape

### savp_model.py 中 GeneratorGivenZ 使用 6/5
...
def with_flat_batch(flat_batch_fn, ndims=4):
    ...
    def fn(x, *args, **kwargs):
        ...
        flat_batch_x = x.reshape(flat_batch_shape)  ### shape= -1HWC 5/9
        ...
        r = nest.map_structure(lambda x: x.reshape(torch.cat([shape[:-(ndims-1)], x.shape[1:]], dim=0)),
                               flat_batch_r)
        return r
    return fn
